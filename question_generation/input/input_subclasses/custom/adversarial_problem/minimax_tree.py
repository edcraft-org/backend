...
class MiniMaxTreeInput(AdversarialEnv, Input):
    ...
    def generate_input(self) -> AdversarialElement:
        """
        Generate input data for the Minimax Tree.
        """
        if self.leaves:
            return self.create_minimax_tree_from_leaves(self.depth, self.num_children, 0)
        else:
            return self.create_minimax_tree_from_scratch(self.depth, self.num_children, 0)

    def create_minimax_tree_from_scratch(self, depth: int, num_children: int, current_depth: int = 0) -> AdversarialElement:
        """
        Recursively create a minimax tree with specified depth and branching factor from scratch.
        """
        ...
        for i in range(num_children):
            child = self.create_minimax_tree_from_scratch(depth, num_children, current_depth + 1)
    ...
    def create_minimax_tree_from_leaves(self, depth: int, num_children: int, current_depth: int = 0) -> AdversarialElement:
        """
        Recursively create a minimax tree with specified depth and branching factor using existing leaves.
        """
        if current_depth + 1 == depth:
        ...
        for i in range(num_children):
            count = self.count
            self.count += 1
            child = self.create_minimax_tree_from_leaves(depth, num_children, current_depth + 1)
            children.append(child)
    ...
    def to_graph(self, label_edges: bool = False) -> str:
        """
        Generate a Graphviz representation of the minimax tree.
    ...
    def generate_options(self) -> 'MiniMaxTreeInput':
        """
        Generate options for the minimax tree by shuffling the leaves.

        Returns:
            List[MiniMaxTreeInput]: The generated options.
        """
